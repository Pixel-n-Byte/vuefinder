<template>
  <div
    class="custom-file-picker-search-bar flex p-1.5 bg-neutral-100 dark:bg-gray-800 border-t border-b border-neutral-300 dark:border-gray-700/50 items-center select-none text-xs"
  >
    <span
      :aria-label="t('Go up a directory')"
          ...
          !isGoUpAvailable() ||
            emitter.emit('vf-fetch', {
              params: {
                ...
                path:
                  breadcrumb[breadcrumb.length - 2]?.path ?? adapter + '://',
              },
            })
        "
    ...
    >
      <svg
        @click="
          emitter.emit('vf-fetch', {
            params: { q: 'index', adapter: data.adapter, path: data.dirname },
          })
        "
    ...
    >
      <svg
        @click="
          emitter.emit('vf-fetch', {
            params: { q: 'index', adapter: data.adapter },
          })
        "
      ...
      <div class="flex leading-5">
        <div v-for="(item, index) in breadcrumb" :key="index">
          ...
          <span
            ...
            @click="
              emitter.emit('vf-fetch', {
                params: { q: 'index', adapter: data.adapter, path: item.path },
              })
            "
            >{{ item.name }}</span
          >
...
const handleDropZone = (e) => {
  ...
  emitter.emit("vf-modal-show", {
    ...
    items: {
      from: draggedItems,
      to: breadcrumb.value[breadcrumb.value.length - 2] ?? {
        path: adapter.value + "://",
      },
    },
  });
