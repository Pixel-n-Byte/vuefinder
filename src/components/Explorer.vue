        ...
        <div class="custom-grid-item-div">
          <div class="relative grid-folder-view-item">
            ...
            <img class="custom-grid-item-file" v-else-if="(item.mime_type ?? '').startsWith('image')"
              :src="item.path.replace('local://', 'https://medias-storage.s3.us-east-2.amazonaws.com/')" :alt="item.basename" />
          ...
          <span class="break-all custom-item-text">{{
...
const title_shorten = (title) =>
  title.replace(/((?=([\w\W]{0,14}))([\w\W]{8,})([\w\W]{8,}))/, "$2..$4");
...
const getSelectedItems = () =>
  ds.value.getSelection().map((el) => JSON.parse(el.dataset.item));
...
const handleDragOver = (e, item) => {
  ...
  if (
    !item ||
    item.type !== "dir" ||
    ds.value.getSelection().find((el) => el == e.currentTarget)
  ) {
...
const setDragSelect = () => {
  ...
  emitter.on("vf-explorer-update", () =>
    nextTick(() => {
      ...
      ds.value.setSelectables(
        document.getElementsByClassName("vf-item-" + randId)
      );
    })
  );
